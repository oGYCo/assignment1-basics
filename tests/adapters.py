...
def run_transformer_block(
    ...
    """
    block = model.Block(d_model, num_heads, d_ff, device=in_features.device, dtype=in_features.dtype, theta=theta, max_seq_len=max_seq_len)
    state = {
...
def run_rmsnorm(
    ...
    """
    norm = model.RMSNorm(d_model, eps, device=weights.device, dtype=weights.dtype)
    state = {"weight": weights}
...
def run_cross_entropy(
    ...
    """
    return model.cross_entropy(inputs, targets)
...
def get_tokenizer(
    ...
    """
    return toknizer.BPETokenizer(vocab, merges, special_tokens)
